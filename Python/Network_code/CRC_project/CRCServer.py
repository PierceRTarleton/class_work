...
class CRCServer(object):
    ...
    def connect_to_server(self):
        ...
        connectServer.connect((self.connect_to_host_addr, self.connect_to_port))
    ...
    def handle_server_message(self, select_key, prefix, command, params):
        ...
        if len(params) < 3:
            ...
            ErrorMessge = self.create_numeric_reply('ERR_NEEDMOREPARAMS', ErrorMessage)
            noError = False
        ...
        if noError:
            ...
            if prefix == None:
                ...
                for server in self.servers_lookuptable:
                    if params[0] != self.servers_lookuptable[server].servername and prefix != self.servers_lookuptable[server].servername:
                        new_message = ':' + self.servername + ' SERVER ' + self.servers_lookuptable[server].servername + ' ' + str(int(self.servers_lookuptable[server].hopcount)+1)

                        if self.servers_lookuptable[server].info != None:
                            new_message += ' :' + self.servers_lookuptable[server].info + '\r\n'
                        else:
                            new_message += '\r\n'
            ...
            message = ':' + self.servername + ' SERVER ' + params[0] + ' ' + str(hop_count) +' :' + params[2] + '\r\n'
            if prefix == None:
    ...
    def handle_user_message(self, select_key, prefix, command, params):
        ...
        if len(params) < 3:
            ...
            if not noError:
                if params[0] not in self.users_lookuptable:
                    ErrorMessage = self.create_numeric_reply('ERR_NICKCOLLISION', ''+ params[0] + ' :Nickname collision KILL from ' + params[3] + '@' + params[1] + '\r\n')
                    self.send_message_to_select_key(select_key, ErrorMessage)
        ...
        if noError:
            ...
            if prefix == None:
                ...
                new_data = 'C', new_user
                self.sel.modify(sock, events, new_data)
                User_message = self.create_numeric_reply('RPL_WELCOME', ':Welcome to the Internet Relay Network ' +params[0]+'!' +params[3]+ '@' + params[1])
            ...
            self.users_lookuptable[params[0]] = new_user
            message = ':' + self.servername + ' USER ' + params[0] + ' ' + params[1] + ' ' + params[2] + ' :' + params[3] + '\r\n'
