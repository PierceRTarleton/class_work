...
class GBNHost():
    ...
    def receive_from_application_layer(self, payload):
        if self.next_seq_num < (self.window_base + self.window_size):
            ack_num = 0
            ack_flag = False
            payload_leangth = len(payload)
            check_sum_holder = 0x0000
            check_sum_pack = pack('!iiH?i%is'%payload_leangth, self.next_seq_num, ack_num, check_sum_holder, ack_flag, payload_leangth, payload.encode())
            check = [0]
            check_sums = 0x0000
            test_added = [0]
            if (payload_leangth + 15)%2 != 0:
                check_sum_pack = check_sum_pack + bytes(1)
            for i in range(0, len(check_sum_pack), 2):
                word = check_sum_pack[i] << 8 | check_sum_pack[i+1]
                if i == 0:
                    check[0] = word
                else:
                    check.append(word)
            for i in range(0, len(check)):
                if i != 4:
                    if i == 0:
                        ...
                        b = check[i+1]
                        c = check[i] + check[i+1]
                        check_sumed = (c & 0xffff) + (c >> 16)
                        i += 2
                    else:
                        if i != 1:
                            a = check[i]
                            c = check_sumed + check[i]
                            check_sumed = (c & 0xffff) + (c >> 16)
            check_sumed = ~check_sumed & 0xffff
            check_sums = check_sumed
            self.unACKed_buffer[self.next_seq_num] = pack('!iiH?i%is'%payload_leangth, self.next_seq_num, ack_num, check_sums, ack_flag, payload_leangth, payload.encode())
    ...
    # i have to first unpack the header then umpack the rest of the packet using the length
    ...
    def receive_from_network_layer(self, byte_data):
        ...
        if True:
            ...
            check_sums = 0
            checking = True
            Checksum_test = byte_data
            if (header[4] + 15)%2 != 0:
                Checksum_test = Checksum_test + bytes(1)
            for i in range(0, len(Checksum_test), 2):
                word = Checksum_test[i] << 8 | Checksum_test[i+1]
                if i == 0:
                    check [0] = word
                else:
                    check.append(word)
                ...
                if i != 4:
                    if i == 0:
                        ...
                        if i != 1:
                            ...
                            check_sumed = (c & 0xffff) + (c >> 16)
            check_sumed = ~check_sumed & 0xffff
            if check[4] == check_sumed:
                if header[4] > 0:
                    payload = unpack("!%is"%header[4], byte_data[15:])[0].decode()
                if header [3]:
                    if header[1] >= self.window_base:
                        packet_ack_num = header[1]
                        old_window_base = self.window_base
                        while packet_ack_num > self.window_base:
                            self.unACKed_buffer.pop(packet_ack_num)
                            packet_ack_num -= 1
                        if header[1] in self.unACKed_buffer:
                            self.unACKed_buffer.pop(header[1])
                        check = [0] * 1
                        self.window_base = header[1]
                        if old_window_base < self.window_base: 
                    ...
                    if header[0] == self.expected_seq_number:
                        ...
                        check_sum_holder = 0x0000
                        payload = None
                        check_sum_pack = pack('!iiH?i', next_seq_number, self.expected_seq_number, check_sum_holder, ack_flag, payload_leangth)
                        checking = False
                        check_sums = 0x0000
                        if (payload_leangth + 15)%2 != 0:
                            check_sum_pack = check_sum_pack + bytes(1)
                        ...
                        check_sumed = ~check_sumed & 0xffff
                        check_sums = check_sumed
                        self.last_ACK_pkt = pack('!iiH?i', next_seq_number, self.expected_seq_number, check_sums, ack_flag, payload_leangth)
                        self.simulator.pass_to_network_layer(self.entity, self.last_ACK_pkt, True)
    ...
    def check_sum(self, byte_data):
        header = unpack("!iiH?i", byte_data[:15])
        recived_packs = [0]
        recived_packs.append(header[0])
        checked = [0] * 1
        check = [0]
        check_sums = 0x0000
        checking = True
